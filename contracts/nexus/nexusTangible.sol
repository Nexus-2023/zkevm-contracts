...
contract NexusBridgeDAO {
    ...
    modifier onlyCurrent(){
        if (msg.sender != address(this)) revert NotCurrentAddress();
        _;
    }

    function _setVariable(bytes32 _slot, uint256 amount) internal {
        assembly {
            sstore(_slot, amount)
        }
    }
    ...
    function validatorsSlashed(uint256 amount) external onlyNexus {
        _setVariable(AMOUNT_SLASHED_SLOT, amount);
    ...
    function redeemRewards(address reward_account) external onlyDAO {
        ...
        _setVariable(REWARDS_CLAIMED_SLOT, rewardsClaimed);
        if (dao_success) {
